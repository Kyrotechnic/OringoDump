...
public abstract class Pipe
{
    ...
    public static Pipe openPipe(final IPCClient ipcClient, final long clientId, final HashMap<String, Callback> callbacks, DiscordBuild... preferredOrder) throws NoDiscordClientException {
        ...
        for (int i = 0; i < 10; ++i) {
            try {
                ...
                pipe.send(Packet.OpCode.HANDSHAKE, new JSONObject().put("v", 1).put("client_id", (Object)Long.toString(clientId)), null);
            ...
            if (i != DiscordBuild.ANY.ordinal()) {
                if (open[i] != null) {
                    try {
                        ...
                        Pipe.LOGGER.debug("Failed to close an open IPC pipe!", (Throwable)ex2);
                    }
                }
            }
        }
    ...
    public void send(final Packet.OpCode op, final JSONObject data, final Callback callback) {
        try {
            ...
            final Packet p = new Packet(op, data.put("nonce", (Object)nonce));
    ...
    static {
        LOGGER = LoggerFactory.getLogger((Class)Pipe.class);
